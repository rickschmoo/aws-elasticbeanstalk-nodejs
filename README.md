...
- Launch a new Elastic Beanstalk Environment
- IAM: Add Permissions to Your Environment's Instances (AmazonDynamoDBFullAccess, AmazonSNSFullAccess), attach to the aws-elasticbeanstalk-ec2-role role
- Deploy bundle to Elastic Beanstalk environment
  - creates DynamicDB
  - creates SNS role
  - Amazon SQS queue
- Fix Confirm subscription misconfig
  - Correct NewSignupEmail in .ebextensions/options.config
  - zip nodejs-tutorial.zip -r * .[^.]*
  - redeploy
  - confirm subscription via email (Subject line: "AWS Notification - Subscription Confirmation")
- Elastic Beanstaclk: restart app server on config change
